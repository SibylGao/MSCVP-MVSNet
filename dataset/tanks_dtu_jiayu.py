...
from dataset.tanks_datapath import *
...
class MVSDataset(Dataset):
    def __init__(self, args, logger=None):
        ...
        self.pair_list_file = self.data_root
        self.logger = logger
        if logger==None:
            import logger
    ...
    def build_list(self,mode):
        ...
        for scan in scan_list:
            self.pair_list_file = self.data_root  + scan + '/pair.txt'
    ...
    def __getitem__(self, idx):
        ...
        ref_img_file = self.data_root + scan + '/images/{:08d}.jpg'.format(ref_view)
        ref_img = read_tanks_img(ref_img_file)

        # src image(s)
        for i in range(self.args.nsrc):
            src_img_file = self.data_root + scan + '/images/{:08d}.jpg'.format(src_views[i])
            # src_img_file = getImageFile(self.data_root,self.args.mode,scan,src_views[i],light_idx)
            src_img = read_tanks_img(src_img_file)

            src_imgs.append(src_img)

        ## 2. Read camera parameters
        # cam_file = getCameraFile(self.data_root,self.args.mode,ref_view)
        cam_file = self.data_root + scan + '/cams/{:08d}_cam.txt'.format(ref_view)
        ref_intrinsics, ref_extrinsics, depth_min, depth_max = read_cam_file_tanks(cam_file)
        for i in range(self.args.nsrc):
            # cam_file = getCameraFile(self.data_root,self.args.mode,src_views[i])
            cam_file = cam_file = self.data_root + scan + '/cams/{:08d}_cam.txt'.format(src_views[i])
            intrinsics, extrinsics, depth_min_tmp, depth_max_tmp = read_cam_file_tanks(cam_file)
