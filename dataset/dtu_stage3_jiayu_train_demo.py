...
class MVSDataset(Dataset):
    def __init__(self, args, logger=None):
        ...
        self.scan_list_file = "/mnt/sharedisk/gaoshiyu/datasets/dtu-train-128/train_demo.txt"
    ...
    def __getitem__(self, idx):
        ...
        ## 1. Read images
        # ref image
        ref_img_file = getImageFile(self.data_root,"train",scan,ref_view,light_idx)
        ref_img = read_img_stage3(ref_img_file)

        # src image(s)
        for i in range(self.args.nsrc):
            src_img_file = getImageFile(self.data_root,"train",scan,src_views[i],light_idx)
            src_img = read_img_stage3(src_img_file)

            src_imgs.append(src_img)

        ## 2. Read camera parameters
        cam_file = getCameraFile(self.data_root,"train",ref_view)
        ref_intrinsics, ref_extrinsics, depth_min, depth_max = read_cam_file_stage3(cam_file)
        for i in range(self.args.nsrc):
            cam_file = getCameraFile(self.data_root,"train",src_views[i])
            intrinsics, extrinsics, depth_min_tmp, depth_max_tmp = read_cam_file_stage3(cam_file)
        ...
        if self.args.mode == "train":
            ...
            depth_file = getDepthFile(self.data_root,"train",scan,ref_view)
            ref_depth = read_depth_stage3(depth_file)
...
def read_cam_file_stage3(filename):
    ...
    intrinsics2[:2, :3] = intrinsics[:2, :3] * 8.   #  
...
def read_depth_stage3(filename):
    depth = np.array(read_pfm(filename)[0], dtype=np.float32)
    h,w = depth.shape[0],depth.shape[1]
    depth = cv2.resize(depth, (w*8, h*8), interpolation=cv2.INTER_NEAREST)
    # read pfm depth file
    return depth

def read_img_stage3(filename):
