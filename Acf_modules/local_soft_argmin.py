...
class LocalSoftArgmin(nn.Module):
    ...
    """
    ## start_disp == 425
    def __init__(self, max_disp, depth_interval, radius, start_disp=425, dilation=1, radius_dilation=1, alpha=1.0, normalize=True):
    ...
    def forward(self, cost_volume, disp_sample=None):
        ...
        # convert to real disparity sample index
        # 这个地方改了，为了适应DTU数据集，depth_interval不是1
        disp_sample = self.start_disp + index_group * self.dilation * self.depth_interval
