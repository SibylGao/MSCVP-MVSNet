...
# from dataset import tanks_dtu_jiayu
...
def save_depth():
    ...
    test_loader = DataLoader(test_dataset, args.batch_size, shuffle=args.eval_shuffle, num_workers=32, drop_last=True)
    ...
    with torch.no_grad():
        ...
        for batch_idx, sample in enumerate(test_loader):
            ...
            # depth_save_path = "./cost_volume_low/"
            # if not os.path.exists(depth_save_path):
            #     os.makedirs(depth_save_path)
            # cost_cpu = cost_reg.detach().clone().cpu().numpy()
            # np.save(depth_save_path+"cost{}_confidence".format(batch_idx),cost_cpu)
            ...
            if batch_idx==178:
                logger.info('-------------debug_test_warning---------------')
...
def check_geometric_consistency(depth_ref, intrinsics_ref, extrinsics_ref, depth_src, intrinsics_src, extrinsics_src):
    ...
    relative_depth_diff = depth_diff / (depth_ref + 1e-10)
...
def filter_depth(dataset_root, scan, out_folder, plyfilename):
    ...
    # pair_file = os.path.join(dataset_root,'Cameras/',scan,'/pair.txt')
    pair_file = dataset_root  + scan + '/cams' + '/pair.txt'
    ...
    for ref_view, src_views in pair_data:
        ...
        ref_intrinsics, ref_extrinsics = read_camera_parameters(
            dataset_root  + scan + '/cams' + '/{:0>8}_cam.txt'.format(ref_view) )
        ...
        ref_img = read_img(os.path.join(dataset_root,scan, "blended_images",'{:08d}.jpg'.format(ref_view))) # Image start from 1.
        ...
        for src_view in src_views:
            ...
            src_intrinsics, src_extrinsics = read_camera_parameters(
                os.path.join(dataset_root,scan,'cams','{:0>8}_cam.txt'.format(src_view)))
        ...
        # at least 3 source views matched, gao modified:at least 5 views matched
...
if __name__ == '__main__':
    ...
    testlist = os.path.join(args.dataset_root,"val_list_gao.txt")
    with open(testlist) as f:
        scans = f.readlines()
        scans = [line.rstrip() for line in scans]

    # root_path = '/mnt/sharedisk/gaoshiyu/CVP-gwc-baseline/intermediate_rescale/'
    ...
    for scan in scans:
        ...
        # print("Start fusion for scan "+str(count))
        filter_depth(args.dataset_root, scan, out_folder, os.path.join(outdir, scan,'cvpmvsnet_l3.ply'))
        # filter_depth(args.dataset_root, scan, out_folder, os.path.join(outdir, 'cvpmvsnet{:0>3}_l3.ply'.format(count)))
