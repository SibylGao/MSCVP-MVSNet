...
class CostRegNet_inception_modified(nn.Module):
    def __init__(self):
        super(CostRegNet_inception_modified, self).__init__()

        self.conv0 = ConvBnReLU3D(4, 16, kernel_size=3, pad=1)
        self.conv0a = ConvBnReLU3D(16, 16, kernel_size=3, pad=1)

        self.conv0b = ConvBnReLU3D(4, 16, kernel_size=3, pad=1)
        self.conv0c = ConvBnReLU3D(32, 16, kernel_size=1, pad=0)
        ...
        self.conv2b = ConvBnReLU3D(32, 32, kernel_size=3, pad=1)
        self.conv2c = ConvBnReLU3D(64, 32, kernel_size=1, pad=0)
    ...
    def forward(self, x):
        ...
        conv0 = self.conv0a(self.conv0(x))
        conv0b = self.conv0b(x)
        conv0c = self.conv0c(torch.cat((conv0,conv0b),1))

        conv1 = self.conv1(conv0c)

        conv2 = self.conv2a(self.conv2(conv1))
        conv2b = self.conv2b(conv1)
        conv2c = self.conv2c(torch.cat((conv2,conv2b),1))

        conv4 = self.conv4a(self.conv4(self.conv3(conv2c)))

        conv5 = conv2c+self.conv5(conv4)
        conv6 = conv0c+self.conv6(conv5)
...
class CostRegNet_gao_modified(nn.Module):
    def __init__(self):
        super(CostRegNet_gao_modified, self).__init__()
    ...
    def forward(self, x):
        ...
        # conv5 = conv2+self.conv5(conv4)
        conv5 = torch.cat((self.conv5(conv4),conv2),1)   #dim=32 bcghw

        # conv6 = conv0+self.conv6(conv5)
        conv6 = torch.cat((self.conv6(conv5),conv0),1)   # bcghw
...
class network(nn.Module):
    def __init__(self, args):
        ...
        self.cost_reg_refine = CostRegNet_inception_modified()
