...
#####################################################gao_added##########################################
        ...
        for level in range(self.args.nscale-2,-1,-1):    #是倒序金字塔
            ...
            depth_list = [2.5,5,10,20]
            # depth_list = depth_list/2
            # print(level)
            mindisparity = depth_up - depth_list[level]
            mindisparity = torch.clamp(mindisparity, 425, 1065)
            maxdisparity = depth_up + depth_list[level]
            maxdisparity = torch.clamp(maxdisparity, 425, 1065)
            if level==self.args.nscale-2:
                depth_hypos = generate_depth_range(mindisparity,maxdisparity)
            else:
                depth_hypos = generate_depth_range(mindisparity,maxdisparity,nhypothesis_init=8)
            ...
            confidence_save_path = "./depth_range_dhs1/"
