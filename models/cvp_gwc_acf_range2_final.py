...
class network(nn.Module):
    def __init__(self, args):
        ...
        self.confidence_gamma = nn.Parameter(torch.ones(1))
        self.confidence_beta = nn.Parameter(torch.ones(1))
    ...
    def forward(self, ref_img, src_imgs, ref_in, src_in, ref_ex, src_ex, \
        ...
        depth_est_list = []
        output = {}
        DTU_variance_scale_gao_defined = 3.3
        ## Feature extraction
...
#####################################################gao_added##########################################
        ...
        for level in range(self.args.nscale-2,-1,-1):    #是倒序金字塔
            ...
            if level==self.args.nscale-2:
                ...
                output["depth_index"] = depth_hypos     #[B,D,H,W]
                # DTU_variance_scale_gao_defined = (maxdisparity_s3 - mindisparity_s3)/32   #[B,H,W]
            else:
            ...
            # np.save(confidence_save_path+"depth_range",cost_cpu)
            # del cost_cpu
            ...
            if level==self.args.nscale-2:
                ...
                confidence = torch.sigmoid(confidence_costs)     #(B,1,H,W)
                # variance = self.confidence_coefficient * (1 - confidence) + self.confidence_init_value   ##s*(1-conf)+epsilon  正态分布的方差
                variance = self.confidence_gamma * (1 - confidence) + self.confidence_beta   ##s*(1-conf)+epsilon  正态分布的方差
                # 1
                # DTU_variance_scale_gao_defined = DTU_variance_scale_gao_defined.unsqueeze(1)
                # variance = variance * DTU_variance_scale_gao_defined
                # 2
                variance = variance * self.DTU_variance_scale
                variances.append(variance)
            ...
            if level==self.args.nscale-2:
                cost_reg_list.append(prob_volume)
        ...
        output["depth_searching_range_list"] = depth_searching_range_list
        del depth_searching_range_list
...
##min_depth和max_depth应该是一张图
def mvsnet_focal_losses_final(costs, depth_gt, variances, mask,index, focal_coefficient):
    #costs:[B,D,H,W]
    #depth_gt[B,H,W]
    #variances[B,1,H,W]
    #max_depthmap[B,1,H,W]
    ...
    scaled_distance = ((-torch.abs(index - depth_gt_mask)) / variances)   #[B,D,H,W]
    # scaled_distance = scaled_distance*mask
    probability_gt = F.softmax(scaled_distance, dim=1)
    probability_gt = probability_gt*mask.unsqueeze(1) + eps    #[B,H,W]
...
def mvsnet_sum_loss_final(costs, depth_est, depth_gt, variances, mask, index, confidence):
    focal_coefficient = 50   #alpha
    lamda_confidence = 80
    lamda_regression = 1

    focal_loss = mvsnet_focal_losses_final(costs, depth_gt, variances, mask, index, focal_coefficient)
    smooth_l1_loss = mvsnet_smooth_l1_loss(depth_est, depth_gt, mask)
    confidence_loss = mvsnet_confidence_loss(confidence)
