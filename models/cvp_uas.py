...
class network(nn.Module):
    def __init__(self, args):
        ...
        # self.cost_reg_refine_second = CostRegNet()
        self.args = args
        ...
        self.stage_configs=[8, 8, 8, 8, 64]
        # self.stage_configs=[8, 32, 64]
        # self.cost_regularization = nn.ModuleList([CostRegNet(),CostRegNet(),CostRegNet()])
    ...
    def forward(self, ref_img, src_imgs, ref_in, src_in, ref_ex, src_ex, \
        ...
        # cost_reg = self.cost_regularization[0](cost_volume)
...
#####################################################gao_added##########################################
        ...
        # exp_variance_list.append(exp_variance)

        exp_variance = None

        if self.args.mode == "test":
            del prob_volume,cost_reg
        ...
        for level in range(self.args.nscale-2,-1,-1):    #是倒序金字塔
            ...
            if exp_variance==None:
                ...
                # UAS depth hypothesis
                depth_hypos = uncertainty_aware_samples(depth_up, exp_variance, ndepth = self.stage_configs[level])
            ...
            # cost_reg2 = self.cost_reg_refine_second(cost_volume)
            # cost_reg2 = self.cost_regularization[level](cost_volume)
