...
class network(nn.Module):
    def __init__(self, args):
        ...
        # self.fast_propagation = FastPropagation_conf()

        self.sarpn_refinenet = Refineblock(num_features=32, kernel_size=3)    #sarpn用的是3的卷积核
...
#####################################################gao_added##########################################
        ...
        for level in range(self.args.nscale-2,-1,-1):    #是倒序金字塔
            ...
            output["depth_secondlayer_init"] = depth
            # gao added: refine network
            depth = self.sarpn_refinenet(depth.unsqueeze(1))
            depth = depth.squeeze(1)
            depth_est_list.append(depth)
