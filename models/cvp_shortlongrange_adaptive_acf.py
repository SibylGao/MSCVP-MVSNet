...
#####################################################gao_added##########################################
        ...
        for level in range(self.args.nscale-2,-1,-1):    #是倒序金字塔
            ...
            if level==self.args.nscale-2:
                ...
                mindisparity_s2 = depth_up - (self.gamma_s2 + 1) * exp_variance - self.beta_s2
                maxdisparity_s2 = depth_up + (self.gamma_s2 + 1) * exp_variance + self.beta_s2

                # del exp_variance

                depth_hypos = generate_depth_range(mindisparity_s2,maxdisparity_s2)

                del mindisparity_s2,maxdisparity_s2
                ...
                d = depth_hypos.shape[1]
                h,w = depth.shape[1], depth.shape[2]
                samp_variance = (depth_hypos - depth.unsqueeze(1).repeat(1,d,1,1)) ** 2
                exp_variance = lamb * torch.sum(samp_variance * prob_volume, dim=1, keepdim=False) ** 0.5

                exp_variance = nn.functional.interpolate(exp_variance[None,:],size=None,scale_factor=2,mode='bicubic',align_corners=None)  #[1,B,H,W]
                exp_variance = exp_variance.squeeze(0)  #[B,H,W]

                mindisparity_s3 = depth_up - (self.gamma_s3 + 1) * exp_variance - self.beta_s3
                maxdisparity_s3 = depth_up + (self.gamma_s3 + 1) * exp_variance + self.beta_s3

                # del exp_variance,depth_up

                depth_hypos = generate_depth_range(mindisparity_s3,maxdisparity_s3,nhypothesis_init = 8)

                del mindisparity_s3,maxdisparity_s3
